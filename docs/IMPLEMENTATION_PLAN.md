...
После P16 доступны P19 (синтетическая история и признаки) и A03 (история/экспорт);
R00 остаётся отдельной обязательной предпроектной карточкой. Браузерная проверка
контакта не доказывает качество модели на реальных людях и не заменяет приёмку P32.
