...
После P15 доступны P16 (контактный UI) и A03 (история/экспорт); R00
остаётся отдельной обязательной предпроектной карточкой. Серверный поток P15
не заменяет браузерную приёмку P16 и не доказывает качество модели на реальных людях.
