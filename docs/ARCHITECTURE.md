...
| Модели и обучающие данные | Детерминированные правила, synthetic fixtures | Перед ML-этапом, отдельным запросом пользователя |
...
Сейчас завершены P01–P04: воспроизводимая среда, общие контракты и границы слоев,
техническое хранение, bootstrap и минимальный HTTP-контур. Точный прогресс, evidence и
следующая доступная задача хранятся в IMPLEMENTATION_PLAN; архитектурная таблица ниже задает
этапы, а не заменяет этот журнал.
...
Завершены P01–P04. Зафиксированы manifests/lock-файлы и проверки Python/Node; реализованы
общие domain/application-типы и исполнимые правила импортов; созданы in-memory и SQLite UoW,
технические idempotency/audit/outbox, первая Alembic-миграция и проверки восстановления.
Bootstrap валидирует конфигурацию и миграцию до старта, собирает только mock-режим и публикует
`/api/v1/health/live` и `/api/v1/health/ready`. Ошибки имеют Problem Details, запросы —
request ID и предел тела. OpenAPI экспортируется детерминированно, из него сгенерированы
frontend-типы.

Readiness сейчас ожидаемо отклоняется из-за отсутствия обязательной политики: P04 проверяет
ее наличие, а P07 добавит схему, безопасную загрузку и валидированное содержимое. Бизнес-
endpoints, пользовательские сессии, анализ риска, web-интерфейс, fixture-сценарии, модели,
